{
  description = "Mamble - Minecraft Paper plugin development environment";
      ...
      devShells = forAllSystems (pkgs:
        let
          ...
          paper-jar = pkgs.writeShellApplication {
            ...
            text = ''
              ...
              UA="mamble/dev (local test server)"
          ...
          # リポジトリのルートで実行すること。
          mamble-dev = pkgs.writeShellApplication {
            name = "mamble-dev";
        ...
        in
        {
          default = pkgs.mkShell {
            ...
            shellHook = ''
              echo "Mamble dev shell"
              echo "  java          : $(java -version 2>&1 | head -n1)"
              echo "  gradle        : $(gradle --version 2>/dev/null | awk '/^Gradle/ {print $2}')"
              echo ""
              echo "  mamble-dev [mc-version]  # ビルド → 配置 → ./run で起動"
