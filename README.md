...
5. 交換機で品目をクリックすると、クレジットを**アイテムに払い出す** (価格は預け入れの単価の **50 倍**。`rewards.yml` の `withdraw-multiplier`)。
...
- **払い出し**: 品目をクリック (1個) / シフトクリック (`exchange.bulk-amount` 個、既定 16)。価格は預け入れの単価 × `withdraw-multiplier` (既定 50)。
